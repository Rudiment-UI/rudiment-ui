...
export const ArticlePage: Story = {
        ...
        <Stack space="2rem">
          ...
          <Prose>
            <p>
              ...
              creeping toward four minutes — and every second of that was felt
              on every pull request.
            </p>
            <h2>The problem with full rebuilds</h2>
            <p>
              Our CI pipeline rebuilt every package from scratch on every push.
              Even a one-line change to a leaf package triggered a cascade of
              full rebuilds up the dependency tree. We needed a way to cache and
              reuse work across builds.
            </p>
            <blockquote>The fastest build is the one you never run.</blockquote>
            <h2>What we tried first</h2>
            <p>
              We experimented with several approaches before landing on our
              current solution:
            </p>
            <ul>
              <li>
                <strong>Turbo remote cache</strong> — effective but required
                infrastructure we weren't ready to maintain.
              </li>
              <li>
                <strong>GitHub Actions cache</strong> — helped, but cache
                invalidation was coarse-grained.
              </li>
              <li>
                <strong>Content-addressed outputs</strong> — the key insight
                that unlocked the most gains.
              </li>
            </ul>
            <h2>Content-addressed caching</h2>
            <p>
              The breakthrough came when we started hashing <em>inputs</em> —
              source files, dependencies, environment variables — and using
              those hashes as cache keys. If the hash matched a previous build,
              we skipped recompilation entirely.
            </p>
            <pre>
              <code>{`const key = sha256([
  ...
  process.env.NODE_ENV,
            ...
            <h2>Results</h2>
            <p>
              After rolling out incremental bundling across all 200+ packages,
              median CI build time dropped from 3m 52s to 1m 31s — a 61%
